...
## 2026-09-03 — v2.5.1: the hidden gesture trace switches itself off

The owner's report from the Pixel: the mobile panel still shows the gesture-trace strip at the bottom, and the setting that switched it off is gone.

Both halves of that are true, and together they are the bug. The trace was switched ON on that phone during the 2.3.0 device rounds, while the toggle was still public (it was, from 1.9.10 through 2.2.1). 2.3.0 then took the toggle off the Settings screen — `public: false`, "kept readable for the next device round" — and Joplin, correctly, kept the stored value. So `joplin.settings.value()` went on answering `true`, `panel.ts` went on putting that `true` on the search-data island, and `panelWebview.js`'s `traceGesture` went on writing the sticky strip, on a build with no switch anywhere to turn it back off. 2.3.0 knew about this case and wrote it down as deliberately left alone (MOBILE.md §7: "the population is mobile users who deliberately enabled a diagnostic on a sideloaded build"). The population turned out to be the owner, and the reasoning was wrong anyway: a diagnostic that a user cannot switch off is not a diagnostic, it is a defect.

**The fix: the build decides, not the value.** `export const gestureTraceAvailable = false` sits next to the registration in `src/core/settings.ts`, and it is the only thing the trace answers to. Every reader of the setting is gated on it — there are exactly two, and the harness now counts them: the island writer in `panel.ts` (`gestureTraceAvailable && !!(await joplin.settings.value(...))`, so a release build never even asks for the stored value) and the self-heal itself. `resetUnavailableGestureTrace()` runs at startup, right after `registerSettings` and deliberately *before* the change handler is registered so its one write cannot re-enter it: a stored `true` is written back to `false`, with a `console.info` line saying so. A stuck profile therefore heals on the first launch of 2.5.1 — and because the reader is gated too, the very first render, the one that precedes that write, already carries the flag off. The registration is untouched (`public: false`, `value: false`), as is every trace point in the webview.

**The dev build is one word.** Flip the constant to `true` — and, only if the toggle should also be reachable from the Settings screen rather than just the strip, `public: true` on the registration — then `npm run dist` and sideload. The self-heal returns on its first line, the island stops short-circuiting, and the trace behaves exactly as it did for the 2.3.0 rounds. Neither flip is ever committed; with one in place `npm test` goes red by design, which is why the recipe in MOBILE.md §7 says `npm run dist` and not `npm test`.

**Pins.** Five new checks. Two runs model the stuck profile exactly — a stored `gestureTrace: true`, once on mobile and once on desktop — and each asserts both halves: `settingWrites` carries exactly one write of that key to `false`, the stored value ends the run off, and the rendered island reads `"gestureTrace":false` with no `"gestureTrace":true` anywhere in the panel html. The 2.3.0 pin keeps its name and gains the build side: the constant is `false` in the shipped source, the reads of the setting across `src/` number exactly two (so a third reader cannot be added later that quietly trusts the stored value), each is shown gated, and the self-heal is called after `registerSettings`. The fifth pins the dev-build behaviour as such — the constant is a single literal, and MOBILE.md names it, so the next device round flips the right word. Three mutations, each caught by the pin written for it: the constant `true` (no reset, the island carries `true` — the documented dev build, and six pins go red); the startup call removed (the scenario pins fail on the missing write); the island gate removed (the *source* pin fails, and only it — at runtime the island is still false there, because the reset lands first, which is exactly why that half is pinned on the source).

Suite: 393 harness checks (five new), all passing. Playwright: 114 tests in 19 files (107 run, 7 opt-in showcase captures) — unchanged and not run on this side; the e2e pass is the verifier's.
