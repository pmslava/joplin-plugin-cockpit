...
export async function buildThemeCss(): Promise<string> {
    ...
    if (mode === "custom") {
        ...
        var customColors: Array<[string, string]> = [
            ["color", String(await joplin.settings.value(customTextColorSettingKey) || "")],
        ...
        for (var pair of customColors) {
            ...
            if (safe) decls.push(`--cockpit-${pair[0]}:${safe}`)
        }
