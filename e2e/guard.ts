 ...
 * harness:
 *   - a machine-wide lock so only ONE E2E run exists at a time, across ALL repos;
...
// One lock for ALL repos on this machine: a run in any sibling repo blocks the others. A directory
// is used because mkdir is atomic across every filesystem; the holder's pid lives in a file inside.
...
/**
 * Acquire the machine-wide lock or fail fast. On a fresh machine (e.g. a CI runner) this trivially
 * succeeds. If another run's pid is still alive we refuse; if the holder is gone we reclaim.
 */
export function acquireLock(): void {
  fs.mkdirSync(path.dirname(LOCK_DIR), { recursive: true });
  try {
    fs.mkdirSync(LOCK_DIR); // atomic create; EEXIST means someone holds (or held) the lock
  } catch (err) {
    ...
    log(`Reclaiming stale E2E lock at ${LOCK_DIR} (holder ${holder ?? 'unknown'} is gone).`);
    try {
    ...
    try {
      ...
      throw new Error(`Could not acquire the E2E lock at ${LOCK_DIR}: ${(err2 as Error).message}`);
    }
  }
  fs.writeFileSync(LOCK_PID_FILE, String(process.pid), 'utf8');
  weOwnLock = true;
  installSignalHandlers();
  log(`Acquired machine-wide E2E lock (pid ${process.pid}).`);
}

/** Release the lock, but only if this process is the one that took it. */
